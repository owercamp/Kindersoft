			...
			<div class="col-md-6">
				<form action="{{ route('city.new') }}" method="PUT">
					<div class="row">
						...
						<div class="col-md-3">
							<button type="submit" class="bj-btn-table-add mb-4 form-control-sm">CREAR</button>
						</div>
					</div>
				</form>
			</div>
			...
			<tbody>
				...
				@foreach($citys as $city)
					<tr>
						<td>{{ $i++ }}</td>
						<td>{{ $city->name }}</td>
						<td><a href="{{ route('city.edit', $city->id) }}" title="EDITAR" class="bj-btn-table-edit edit-city"><i class="fas fa-edit"></i></a></td>
						<td><a href="{{ route('city.delete', $city->id) }}" title="ELIMINAR" class="bj-btn-table-delete" onclick="return confirm('Si borra esta ciudad, se eliminarán las localidades y barrios relacionados. ¿Desea borrar la ciudad con sus localidades y barrios?')"><i class="fas fa-trash-alt"></i></a></td>
					</tr>
				@endforeach
			</tbody>
		</table>
