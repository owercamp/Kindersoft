    ...
    <body>
        ...
        <section>
            ...
            <article class="art-info">
                <h2><b>HORARIO DE FECHA:</b> </h2><br>                
                <h2>{{ $dateSelected }}</h2><br>       
            </article>
        </section>
        <br>
        <hr>
        <table border="1" width="100%" style="border-collapse: collapse;">
            ...
            <tbody>
                ...
                @foreach($resultFilter as $hourSpace)
                    @php
                        $day = getDayString($hourSpace->hwDay);

                        $hourInitialSeparated = explode(':',trim($hourSpace->hwHourInitial));
                        $timeHourInitial = date('A', strtotime($hourSpace->hwHourInitial));
                        $hourInitial = date('h', strtotime(trim($hourSpace->hwHourInitial))) . ':' . $hourInitialSeparated[1] . ' ' . $timeHourInitial;

                        
                        $hourFinalSeparated = explode(':',trim($hourSpace->hwHourFinal));
                        $timeHourFinal = date('A', strtotime(trim($hourSpace->hwHourFinal)));
                        $hourFinal = date('h', strtotime(trim($hourSpace->hwHourFinal))) . ':' . $hourFinalSeparated[1] . ' ' . $timeHourFinal;
                    @endphp
                    <tr>
                        <td class="infoDate">{{ $day }} <br> {{ $hourSpace->hwDate }}</td>
                        <td class="infoDate">{{ $hourInitial }} A {{ $hourFinal }}</td>
                        @if($day == 'LUNES')
                            <td class="infoDay">
                                <b>Curso:</b><br>{{ $hourSpace->name }} <br>
                                <b>Espacio:</b><br>{{ $hourSpace->asNumber }}-{{ $hourSpace->asSpace }} <br>
