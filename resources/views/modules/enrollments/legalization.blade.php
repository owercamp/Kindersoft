		...
		<form action="{{ route('legalizationEnrollment.new') }}" method="POST" class="mb-3" autocomplete="off">
						...
						<div class="col-md-12">
							<div class="form-group">
								...
								<select name="legStudent_id" class="form-control form-control-sm" required>
									...
									@for($i = 0; $i < count($studentsUnique); $i++)
										<option value="{{ $studentsUnique[$i][0] }}">{{ $studentsUnique[$i][1] }}</option>
									@endfor
								</select>
							</div>
							<div class="row">
			...
			<div class="row">
				<div class="col-md-12 text-center">
					<div class="form-group">
						<button type="submit" class="bj-btn-table-add form-control-sm mt-3">GUARDAR LEGALIZACION</button>
					</div>
				</div>
			</div>
		</form>
	...
	<script>
		...
		$('select[name=legStudent_id]').on('change',function(e){
			...
			if(selectedStudent !== null && selectedStudent !== ''){
				$.get("{{ route('legStudentSelected') }}",{selectedStudent: selectedStudent},function(objectStudent){
		...
		$('select[name=legAttendantfather_id]').on('change',function(e){
			...
			if(selectedAttendant !== null && selectedAttendant !== ''){
				$.get("{{ route('legAttendantSelected') }}",{selectedAttendant: selectedAttendant},function(objectAttendant){
