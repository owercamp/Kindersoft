...
<div class="row card">
  <div class="card-header">
    <div class="row text-center">
      <div class="col-md-6">
        <h6 class="text-muted">MODIFICACION DE ACUDIENTE: <b>{{ $attendant->threename }},
            {{ $attendant->firstname }}</b></h6>
      </div>
      <div class="col-md-6">
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <select class="form-control form-control-sm" name="typedocument_id_edit" required="required">
                  ...
                  <option value="{{ $document->id }}" selected="selected">
                    {{ $document->type }}
                  </option>
                  @else
                ...
                @if ($document->type === '')
                <small for="typedocument_id_edit" class="text-muted">Actualmente es
                  <b>{{ __('Dato vacio') }}</b></small>
                @else
                <small for="typedocument_id_edit" class="text-muted">Actualmente es
                  <b>{{ __('') }}</b></small>
                @endif
              ...
              <div class="col-md-6">
                ...
                <small class="text-muted">Actualmente es
                  <b>{{ $attendant->numberdocument }}</b></small>
              </div>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <select class="form-control form-control-sm" name="bloodtype_id_edit" required>
                  ...
                  <option value="{{ $bloodtype->id }}" selected>{{ $bloodtype->group }}
                    {{ $bloodtype->type }}
                  </option>
                  @else
                  <option value="{{ $bloodtype->id }}">{{ $bloodtype->group }}
                    {{ $bloodtype->type }}
                  </option>
                  @endif
                  @endforeach
              ...
              <div class="col-md-6">
                ...
                <small id="genderActualView" for="gender_edit" class="text-muted">Actualmente es
                  <b>{{ $attendant->gender }}</b></small>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="form-group">
            <div class="row">
              <div class="col-md-6">
                ...
                <small class="text-muted">Referencia actual <b>{{ __('Dato vacio') }}</b></small>
                @else
                <small class="text-muted">Referencia actual
                  <b>{{ $attendant->locationcompany_id }}</b></small>
                @endif
              </div>
              <div class="col-md-6">
                ...
                @else
                <small class="text-muted">Referencia actual
                  <b>{{ $attendant->dictrictcompany_id }}</b></small>
                @endif
              </div>
            </div>
          </div>
    ...
    <div class="card-footer">
      <div class="row">
        <div class="col-md-6 text-center">
          <button type="submit" id="saveAttendant" class="btn btn-outline-primary form-control-sm">GUARDAR
            CAMBIOS</button>
        </div>
        <div class="col-md-6 text-center">
...
<script>
  ...
  function countryHome(id, country) {
    $.ajax({
      "_token": "{{ csrf_token() }}",
      url: "{{ route('apiInitials') }}",
      type: "POST",
      dataType: "JSON",
      data: {
        id: id,
      ...
      success(response) {
        response['departamentos'].map((e) => {
          if (e.codiDepartament == response['acudiente'][0]['departamenthome_id']) {
            $("#departament_home_id_edit").append(
              `<option value="${e.codiDepartament}" selected>${e.diminutiveDepartment} - ${e.nomDepartament}</option>`
            )
          } else {
            $("#departament_home_id_edit").append(
              `<option value="${e.codiDepartament}">${e.diminutiveDepartment} - ${e.nomDepartament}</option>`
        ...
        response['ciudades'].map((e) => {
          if (e.cod_ciud_prefijoopcional == response['acudiente'][0]['cityhome_id']) {
            $("#city_home_id_edit").append(
              `<option value="${e.cod_ciud_prefijoopcional}" selected>${e.nom_ciud_prefijoopcional}</option>`
            )
          } else {
            $("#city_home_id_edit").append(
              `<option value="${e.cod_ciud_prefijoopcional}">${e.nom_ciud_prefijoopcional}</option>`
            )
          }
        })

        response['postales'].map((e) => {
          if (e == response['acudiente'][0]['postalhome_id']) {
            $("#postal_home_id_edit").append(
  ...
  $("#departament_home_id_edit").change((e) => {
    ...
    $.ajax({
      ...
      success(response) {
        $("#city_home_id_edit").append(`<option value="">Seleccione una ciudad</option>`);
        response.map((e) => {
          $("#city_home_id_edit").append(
            `<option value="${e.cod_ciud_prefijoopcional}">${e.nom_ciud_prefijoopcional}</option>`
          );
  ...
  $("#city_home_id_edit").change((e) => {
    ...
    $.ajax({
      ...
      success(response) {
        ...
        response.map((e) => {
          $("#postal_home_id_edit").append(`<option value="${e}">${e}</option>`);
        })
      }
    })
  });
      ...
      for (var i = 0; i < count; i++) {
        if (locationcompany_id_hidden != '') {
          if (locationcompany_id_hidden == locationObject[i]['id']) {
            $('#locationcompany_id_edit').append("<option value=" + locationObject[i]['id'] +
              " selected>" + locationObject[i]['name'] + "</option>");
          } else {
            $('#locationcompany_id_edit').append("<option value=" + locationObject[i]['id'] + ">" +
              locationObject[i]['name'] + "</option>");
          }
        } else {
          $('#locationcompany_id_edit').append('<option value=' + locationObject[i]['id'] + '>' +
            locationObject[i]['name'] + '</option>');
        ...
        for (var i = 0; i < count; i++) {
          if (dictrictcompany_id_hidden != '') {
            if (dictrictcompany_id_hidden == districtObject[i]['id']) {
              $('#dictrictcompany_id_edit').append("<option value=" + districtObject[i][
                'id'
              ] + " selected>" + districtObject[i]['name'] + "</option>");
            } else {
              $('#dictrictcompany_id_edit').append("<option value=" + districtObject[i][
                'id'
              ] + ">" + districtObject[i]['name'] + "</option>");
            }
          } else {
            $('#dictrictcompany_id_edit').append('<option value=' + districtObject[i][
              'id'
