  ...
  <table id="tablelocations" class="table table-hover text-center" width="100%">
    ...
    <tbody>
      ...
      <tr>
        <td>{{ $i++ }}</td>
        <td>{{ $location->name }}</td>
        <td><a href="{{ route('location.edit', $location->id) }}" title="EDITAR" class="btn btn-outline-primary rounded-circle"><i class="fas fa-edit"></i></a></td>
        <td><a href="{{ route('location.delete', $location->id) }}" title="ELIMINAR" class="btn btn-outline-tertiary rounded-circle " onclick="return confirm('Si borra esta localidad, se eliminarán los barrios relacionados. ¿Desea borrar la localidad y sus barrios?')"><i class="fas fa-trash-alt"></i></a></td>
      </tr>
      @endforeach
    </tbody>
  </table>
