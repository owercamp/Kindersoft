	...
	<div class="col-md-12">
		...
		<table id="tablecustomers" class="table table-responsive" width="100%">
			...
			<tbody>
				...
				@for($i = 0; $i < count($customerInactive);$i++)
					<tr>
						...
						<td>
							<a href="#" title="AGENDAR CITA" class="bj-btn-table-add scheduling-link">
							...
							<a href="{{ route('customer.edit', $customerInactive[$i][0]) }}" title="EDITAR" class="bj-btn-table-edit"><i class="fas fa-edit"></i></a>
							<a href="{{ route('customer.delete', $customerInactive[$i][0]) }}" title="ELIMINAR" class="bj-btn-table-delete" onclick="return confirm('- Se eliminará el cliente, tenga en cuenta que se eliminarán los agendamientos PENDIENTES que tenga el cliente, ¿Desea eliminar el cliente?')"><i class="fas fa-trash-alt"></i></a>
						</td>
					</tr>
				@endfor
			</tbody>
	      			...
	      			<form action="{{ route('scheduling.only') }}" method="POST" autocomplete="off">
	      				...
	      				<div class="row">
	      					<div class="col-md-12 text-center">
	      						<button type="submit" class="bj-btn-table-add">AGENDAR</button>
	      					</div>
	      				</div>
	      			</form>
	      		</div>
