		...
		<form action="{{ route('weeklyTracking.new') }}" method="POST">
			...
			<div class="row">
				<div class="col-md-12 text-center d-flex justify-content-center">
					...
					<button type="submit" class="bj-btn-table-add form-control-sm btn-save" style="display: none;">GUARDAR SEGUIMIENTO</button>
				</div>
			</div>
		</form>
	</div>
	...
	<script>
			...
			if(courseSelected != ''){
				$.get("{{ route('getInfoCourseConsolidated') }}",{courseSelected: courseSelected},function(objectInfo){
					if(objectInfo != null && objectInfo != ''){
						...
						$.get("{{ route('getStudentCourseSelected') }}",{courseSelected: courseSelected},function(objectStudent){
			...
			if(periodSelected != '' && courseSelected != ''){
				$.get("{{ route('getRangePeriod') }}",{ periodSelected: periodSelected },function(objectDates){
					if(objectDates != null && objectDates != ''){
						...
						$.get("{{ route('getRangeWeek') }}",{periodSelected: periodSelected, courseSelected: courseSelected},function(objectWeek){
		...
		$('select[name=wtIntelligence_id]').on('change',function(e){
			...
			if(intelligenceSelected != ''){
				$.get("{{ route('getDescriptionIntelligence') }}",{intelligenceSelected: intelligenceSelected},function(objectDescription){
				...
				$.get("{{ route('getAchievement') }}",{intelligenceSelected: intelligenceSelected},function(objectAchievement){
		...
		$('select[name=wtChronological]').on('change',function(e){
			...
			if(numberWeek != '' && courseSelected != '' && periodSelected != ''){
				$.get("{{ route('getBasesFromWeek') }}",{ numberWeek: numberWeek, periodSelected: periodSelected, courseSelected: courseSelected },function(objectBases){
		...
		function getAchievements(idIntelligence){
			$.get("{{ route('getAchievement') }}",{intelligenceSelected: idIntelligence},function(objectAchievement){
