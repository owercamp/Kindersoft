  ...
  <div class="col-md-12">
      ...
      <div class="col-md-6">
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <input class="form-control form-control-sm text-center" name="infoPlusmount" pattern="[0-9.]" readonly
      ...
      <div class="col-md-6">
        <form action="{{ route('accounts.tramited') }}" method="GET">
          <div class="form-group">
            ...
            <input class="form-control form-control-sm" name="xlsYear" readonly required type="hidden">
            <input class="form-control form-control-sm" name="xlsMount" readonly required type="hidden">
            <button class="btn btn-outline-success form-control-sm btn-facturesTramited" type="submit">GENERAR
              EXCEL</button>
            <span class="info-excel" hidden style="color: red; font-size: 10px; font-weight: bold;"></span>
          </div>
        </form>
      </div>
    </div>
    ...
    <div class="row m-3 text-center">
      <div class="col-md-6">
        ...
        <div class="form-inline m-0 border p-1" style="background: #ccc;">
          <small class="text-muted" style="color: #000;">AÑO DE CONSULTA:</small>
          <select class="form-control form-control-sm ml-3" name="yearSelected">
            <option value=""></option>
            ...
            <option selected value="{{ $yearnow }}">{{ $yearnow }}</option>
        ...
        <input class="form-control form-control-sm" disabled name="mountSelected" type="hidden"
          value="{{ $mountnow }}">
      </div>
      <div class="col-md-6">
    ...
    <div class="row bj-spinner text-center">
      <div class="col-md-12">
        <div align="center" class="spinner-border" role="status">
          <span align="center" class="sr-only">Procesando...</span>
        </div>
      </div>
    </div>
    ...
    <div class="row sectionTable" style="display: none;">
      <div class="col-md-12">
        ...
        <table class="table text-center" id="tableAccount" width="100%">
          <thead>
            <tr>
              <th>ALUMNO</th>
              <th>GRADO</th>
        ...
        <div class="modal-body p-2">
          <div class="row">
            <div class="col-md-4 border-right">
              <div class="form-group">
                <div class="input-group">
                  ...
                  <input class="form-control form-control-sm text-center" disabled name="idLegalization-modal" required
                    style="font-weight: bold;" type="text">
                </div>
              </div>
              <div class="form-group" style="font-size: 12px;">
                <hr>
                <small class="text-muted" style="font-size: 14px;">ALUMNO:</small>
                <hr>
                <p class="nameStudent-modal"></p>
                <p class="nameGrade-modal"></p>
                <hr>
                <small class="text-muted" style="font-size: 14px;">ACUDIENTE/S:</small>
                ...
                <small class="text-muted" style="font-size: 14px;">LEGALIZACION DE CONTRATO:</small>
            ...
            <div class="col-md-8 border-left text-center">
              <h6 class="border p-4 text-center">ESTADO DE CUENTAS - FACTURACION</h6>

              <table class="table-hover table-modal table text-center" style="font-size: 12px;" width="100%">
                <thead>
                  <tr>
                    <th>CONCEPTO</th>
              ...
              <a class="btn btn-outline-success link-saveAccountsStatus" href="#" style="display: none;"><span
                  hidden></span>FACTURAR</a>
            </div>
          </div>
        </div>
      </div>
        ...
        success: function(response) {
          ...
          for (var i = 0; i < response.length; i++) {
            if (response[i][0] == 'ALUMNO') {
              join += 'nameStudent=' + response[i][1] + '&typeStudent=' + response[i][2] + '&numberStudent=' +
                response[i][3] + '&';
            }
            if (response[i][0] == 'PADRE') {
              join += 'nameFather=' + response[i][1] + '&typeFather=' + response[i][2] + '&numberFather=' +
                response[i][3] + '&';
            }
            if (response[i][0] == 'MADRE') {
              join += 'nameMother=' + response[i][1] + '&typeMother=' + response[i][2] + '&numberMother=' +
                response[i][3] + '&';
            }

            if (response[i][0] == 'CONCEPTO') {
              join += 'conDate' + countConcepts + '=' + response[i][1] + '&conConcept' + countConcepts + '=' +
                response[i][2] + '&conValue' + countConcepts + '=' + response[i][3] + '&';
              countConcepts++;
            }

            if (response[i][0] == 'FACTURA') {
              join += 'codeFacture=' + response[i][1] + '&dateFacture=' + response[i][2] + '&totalFacture=' +
                response[i][3] + '&legalization=' + response[i][4] + '&';
            }
          ...
          if (join.length > 50) {
            ...
            $('.link-saveAccountsStatus').parent('div').append(
              "<p style='color: red; font-size: 10px; font-weight: bold;'>NO ES POSIBLE FACTURAR SIN NUMERACION, DIRIJASE A ==> FINANCIERO >> DOCUMENTOS CONTABLES >> Información general; y complete el formulario de numeración</p>"
            );
          }
        }
      });
      ...
      $.get("{{ route('accounts.get') }}", {
        ...
        if (count > 0) {
          ...
          for (var i = 0; i < count; i++) {
            ...
            rownow.add(
              [
                "<span hidden>" + year + '-' + mount + '-' + objectAccounts[i][0] + "</span>" + objectAccounts[i][
                  2
                ],
                objectAccounts[i][3],
                objectAccounts[i][4],
