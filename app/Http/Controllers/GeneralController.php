...
class GeneralController extends Controller
{
  ...
  {
    $garden = Garden::select('garNamelogo', 'garCode')->get();
    $logo = asset('storage/garden/' . $garden[0]['garNamelogo']);
    $qr = asset('storage/garden/' . $garden[0]['garCode']);
    $files = filesGarden::select('header','footer','format')->get();
    $header = asset('storage/garden/' . $files[0]['header']);
    $footer = asset('storage/garden/' . $files[0]['footer']);
    $formato = asset('storage/garden/' . $files[0]['format']);
    // dd($logo);
    return view('modules.garden.companyLogo', compact('logo', 'qr', 'header', 'footer', 'formato'));
  }


  public function logoCompany(Request $request)
  {
    if ($request->hasFile('logo')) {
      $logo = $request->file('logo');
      Storage::disk('kindersoft')->putFileAs('storage/garden/', $logo, $logo->getClientOriginalName());
      DB::table("garden")->where('garId', 1)->update(['garNamelogo' => $logo->getClientOriginalName()]);
    }
    if ($request->hasFile('codigo')) {
      $codigo = $request->file('codigo');
      Storage::disk('kindersoft')->putFileAs('storage/garden/', $codigo, $codigo->getClientOriginalName());
      DB::table("garden")->where('garId', 1)->update(['garCode' => $codigo->getClientOriginalName()]);
    }
    if ($request->hasFile('header')) {
      $header = $request->file('header');
      Storage::disk('kindersoft')->putFileAs('storage/garden/', $header, $header->getClientOriginalName());
      DB::table("files_gardens")->where('id', 1)->update(['header' => $header->getClientOriginalName()]);
    }
    if ($request->hasFile('footer')) {
      $footer = $request->file('footer');
      Storage::disk('kindersoft')->putFileAs('storage/garden/', $footer, $footer->getClientOriginalName());
      DB::table("files_gardens")->where('id', 1)->update(['footer' => $footer->getClientOriginalName()]);
    }
    if ($request->hasFile('formato')) {
      $formato = $request->file('formato');
      Storage::disk('kindersoft')->putFileAs('storage/garden/', $formato, $formato->getClientOriginalName());
      DB::table("files_gardens")->where('id', 1)->update(['format' => $formato->getClientOriginalName()]);
    }

    return redirect()->route('companylogo')->with("Success", "Imagen Actualizada");
  }
}
