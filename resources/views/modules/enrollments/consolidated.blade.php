		...
		<div class="row">
			<div class="col-md-6">
				...
				<div class="row text-center">
					<a href="{{ route('consolidatedEnrollment.new') }}" class="bj-btn-table-add form-control-sm">CREAR NUEVA MATRICULA</a>
				</div>
			</div>
			<div class="col-md-6">
						...
						<td>
							@if($itemEnrollment->conenStatus == 'PENDIENTE' || $itemEnrollment->conenStatus == 'PENDIENTE DE DOCUMENTOS')
								<form action="{{ route('getDocumentsPending') }}">
									<a href="#" class="bj-btn-table-edit updateConsolidatedEnrollment-link" title="CONTINUAR PROCESO">
									...
									<button type="submit" class="bj-btn-table-delete" title="DESCARGAR PDF"><i class="fas fa-file-pdf"></i></button>
								</form>
							@elseif($itemEnrollment->conenStatus == 'COMPLETADO')
								<a href="#" class="bj-btn-table-add detailsConsolidatedEnrollment-link" title="VER DETALLES">
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
					...
					<div class="row">
						<div class="col-md-5 border-right">
							...
							<form id="formForceFinalConsolidated" action="{{ route('saveConsolidatedWithoutDocuments') }}" method="POST">
								...
								<button type="submit" class="bj-btn-table-edit btn-enrollWithoutDocuments">MATRICULAR SIN DOCUMENTOS</button>
							</form>
						</div>
							...
							<div id="checkItems" class="row">
								<div class="col-md-12">
									<form id="formForUpdateConsolideEnroll" action="{{ route('consolidatedEnrollment.update') }}" method="POST">
					        			...
					        			<button id="btnFormUpdateConsolidatedEnrollment" type="submit" class="bj-btn-table-add mx-3 form-control-sm">GUARDAR PROCESO</button>
					        		</form>
								</div>
							</div>
						</div>
				...
				<div class="modal-footer text-center">
					<button type="button" class="bj-btn-table-delete form-control-sm" data-dismiss="modal">CERRAR</button>
				</div>
			</div>
		</div>
	</div>
...
@section('scripts')
	<script>
		...
		$('.updateConsolidatedEnrollment-link').on('click',function(){
			...
			$.get("{{ route('getTypeDocument') }}",{id: $(this).find('span:nth-child(6)').text() },function(objectTypedocument){
			...
			$.get("{{ route('getCity') }}",{id: $(this).find('span:nth-child(12)').text() },function(objectCity){
			...
			$.get("{{ route('getLocation') }}",{id: $(this).find('span:nth-child(13)').text() },function(objectLocation){
			...
			$.get("{{ route('getDistrict') }}",{id: $(this).find('span:nth-child(14)').text() },function(objectDistrict){
			...
			$.get("{{ route('getTypeBlood') }}",{id: $(this).find('span:nth-child(15)').text() },function(objectTypeblood){
			...
			$.get("{{ route('getHealth') }}",{id: $(this).find('span:nth-child(17)').text() },function(objectHealth){
			...
			var estAddress = $(this).find('span:nth-child(11)').text();			var estTypeblood = $(this).find('span:nth-child(15)').text();
		...
		$('#btnFormUpdateConsolidatedEnrollment').on('click',function(e){
			...
			if(idStudent > 0 && idStudent !== '' && idConsolidated > 0 && idConsolidated !== ''){
				...
				$.post("{{ route('updateConsolidatedEnrollment') }}", {jsonItems}, function(objectResponse){
