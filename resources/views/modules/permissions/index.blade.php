  ...
  <table id="tableroles" class="table table-striped table-bordered text-center" style="width:100%;">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{ $role->name }}</td>
        <td>{{ $role->name }}</td>
        <td><a href="{{ route('role.edit', $role->id) }}" title="EDITAR" class="btn btn-outline-primary rounded-circle"><i class="fas fa-edit"></i></a></td>
        <td><a href="{{ route('role.delete', $role->id) }}" title="ELIMINAR" class="btn btn-outline-tertiary rounded-circle "><i class="fas fa-trash-alt"></i></a></td>
      </tr>
      @endforeach
    </tbody>
    <tfoot>
