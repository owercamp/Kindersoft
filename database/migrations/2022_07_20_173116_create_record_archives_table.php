...
class CreateRecordArchivesTable extends Migration
{
	/**
	 * Run the migrations.
	 *
	...
	public function up()
	{
		Schema::create('record_archives', function (Blueprint $table) {
			...
			$table->string('foto')->default('photodefault.png');
			...
			$table->bigInteger('tiposangre')->unsigned();
			...
			$table->bigInteger('docacu1')->unsigned();
			...
			$table->bigInteger('rhacu1')->unsigned();
			$table->enum('sexoacudiente1', ['MASCULINO', 'FEMENINO']);
			$table->string('nombreacudiente2');
			$table->bigInteger('docacu2')->unsigned();
			...
			$table->bigInteger('rhacu2')->unsigned();
			$table->enum('sexoacudiente2', ['MASCULINO', 'FEMENINO']);
			...
			$table->integer('migracion')->default(1);
			$table->string('periodo_escolar',100);
			...
			//LLAVES FORANEAS
			$table->foreign('tiposangre')->references('id')->on('bloodtypes');
			$table->foreign('rhacu1')->references('id')->on('bloodtypes');
			$table->foreign('rhacu2')->references('id')->on('bloodtypes');
			$table->foreign('docacu1')->references('id')->on('documents');
			$table->foreign('docacu2')->references('id')->on('documents');
		});
	}
