	...
	<div class="col-md-12">
		<div class="row">
			<div class="col-md-12 text-center">
				<!-- Mensajes de circulares administrativas -->
				@if(session('SuccessCircularadmin'))
				    <div class="alert alert-success">
				        {{ session('SuccessCircularadmin') }}
				    </div>
				@endif
				@if(session('SecondaryCircularadmin'))
				    <div class="alert alert-secondary">
				        {{ session('SecondaryCircularadmin') }}
				    </div>
				@endif
			</div>
		</div>
		@php $datenow = Date('Y-m-d') @endphp
		<form action="{{ route('circularadministrative.pdf') }}" method="GET" class="mt-4">
			@csrf
			<div class="row">
				<div class="col-md-12 text-center">
					<h4>CIRCULAR ADMINISTRATIVA</h4>
				</div>
			</div>
			<div class="row">
				<div class="col-md-4">
			...
			<div class="row">
				...
				<div class="col-md-6 text-center">
					<button type="submit" class="bj-btn-table-delete form-control-sm my-3 btnDownload">GENERAR CIRCULAR</button>
				</div>
			</div>
		</form>
	</div>
...
@section('scripts')
	<script>
		...
		$('select[name=cirBody_id]').on('change',function(e){
			...
			if(selected != ''){
				$.get("{{ route('getBodyselected') }}",{body_id: selected},function(objectBody){
					if(objectBody != null){
	    ...
	    function getCompletesNumber(number){
	        ...
	        switch (len) {
	            case 1: return '0000' + number; break;
	            case 2: return '000' + number; break;
	            case 3: return '00' + number; break;
	            case 4: return '0' + number; break;
