  ...
  function updateAttendant(Request $request, $id)
  {
    try {
      ...
      if ($attendantUpdate == null) {
        $attendant = Attendant::find($id);
        $emailOld = $attendant->emailone;
        // dd($emailOld, $request->all(), $attendant);
        ...
        if ($attendant->save()) {
          /**
           * ? se realiza la consulta del correo del acudiente 1
           */
          $emailAttendant1 = DB::table('formsadmissions')->where([['correoacudiente1',$emailOld],['documentoacudiente1',$attendant->numberdocument]]);
          if ($emailAttendant1->count() > 0) {
            $emailAttendant1->get();
            $emailAttendant1->update(['correoacudiente1' => mb_strtolower(trim($request->emailone_edit))]);
          }
          /**
           * ?se modifica la consulta para buscar en el correo del acudiente 2
           */
          $emailAttendant2 = DB::table('formsadmissions')->where([['correoacudiente2',$emailOld],['documentoacudiente2',$attendant->numberdocument]]);
          if($emailAttendant2->count() > 0){
            $emailAttendant2->get();
            $emailAttendant2->update(['correoacudiente2' => mb_strtolower(trim($request->emailone_edit))]);
          }
        }
