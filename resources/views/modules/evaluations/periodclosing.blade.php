		    ...
		    <div class="modal-content px-4 py-4">
		    	<form action="{{ route('weeklyTracking.save') }}" method="POST">
			    			...
			    			<div class="row">
			    				<div class="col-md-12">
			    					...
			    					<div class="form-group text-center">
			    						<button type="button" class="bj-btn-table-add form-control-sm btn-addObservation" title='AGREGUE OBSERVACIONES A LA TABLA PARA ESTE PERIODO'>AGREGAR</button>
			    	...
			    	<div class="modal-body">
			    		...
			    		<button type="submit" class="bj-btn-table-add mx-3 form-control-sm btn-saveDefinitive">CONTINUAR Y GUARDAR</button>
...
@section('scripts')
	<script>
		...
		$('select[name=pcStudent]').on('change',function(e){
			...
			if(studentSelected != ''){
				var infoStudent = [];
				$.get("{{ route('getInfoBasicStudentPeriodClosing') }}",{studentSelected: studentSelected},function(response){
		...
		$('select[name=pcPeriod]').on('change',function(e){
			...
			if(periodSelected != '' && studentSelected != ''){
				$.get("{{ route('getAchievementAll') }}",{ apId: periodSelected , stId: studentSelected },function(objectTrackings){
						...
						contentAll += "<table class='table text-center my-3 tableUp' width='100%'>" +
											"<thead style='background-color: #ccc'>" +
												"<tr>" +
													"<th><h5>DEFINITIVA HASTA LA FECHA:</h5></th>" +
													"<th><h6 class='noteFinal' style='font-size: 20px; font-weight: bold;'>0</h6></th>" +
													"<th><button type='button' class='bj-btn-table-add form-control-sm btn-saveNotes'>GUARDAR PROCESO</button></th>" +
												"</tr>"
											"</thead>" +
										"</table>";
		...
		$('.sectionsBases').on('click','.btn-saveNotes',function(){
			...
			$.get("{{ route('getIntelligenceFromArray') }}",{ ids: idsIntelligences },function(objectIntelligences){
						...
						for(var o = 0; o < count; o++){
							$('.tableObservations').find('tbody').append(
								"<tr class='" + objectObservations[o][0] + "' data-idObservation='" + objectObservations[o][0] + "'>" +
									...
									"<td>" +
										"<button type='button' class='bj-btn-table-delete form-control-sm btn-deleteObservations'><i class='fas fa-trash-alt'></i></button>" +
									"</td>" +
								"</tr>"
							);
						}
		...
		$('select[name=taIntelligence_id]').on('change',function(e){
			...
			if(value != ''){
				$.get("{{ route('getObservationsFromIntelligence') }}",{ idIntelligence: value }, function(objectsObservations){
		...
		$('.tableObservations').on('click','.btn-deleteObservations',function(){
			...
			$(this).removeClass('bj-btn-table-delete');
			$(this).addClass('btn-addObservations');
			$(this).addClass('bj-btn-table-edit');
		...
		$('.tableObservations').on('click','.btn-addObservations',function(){
			...
			$(this).addClass('bj-btn-table-delete');
			$(this).removeClass('btn-addObservations');
			$(this).removeClass('bj-btn-table-edit');
			var classRow = $(this).parents('tr').attr('data-idObservation');
		...
		function getAchievementsFromWeek(chId){
			...
			$.get("{{ route('getAchievementFromWeek') }}",{chId:chId,selectedStudent:selectedStudent},function(objectAchievements){
