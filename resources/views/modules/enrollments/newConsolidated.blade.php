	...
	<div class="col-md-12">
		<div class="row text-center my-2">
			<div class="col-md-6">
				...
				<diw class="row text-center">
					<a href="{{ route('consolidatedEnrollment') }}" class="bj-btn-table-delete form-control-sm">VER TODAS LAS MATRICULAS</a>
				</diw>
			</div>
			<div class="col-md-6">
		...
		<form id="formConsolidatedEnrollment" action="" method="POST">
			...
			<div class="row text-center">
				<button id="btnFormSaveConsolidatedEnrollment" type="submit" class="bj-btn-table-add form-control-sm">GUARDAR PROCESO</button>
			</div>
		</form>
	</div>
@endsection
	...
	<script>
		...
		$('select[name=conenStudent]').on('change',function(e){
			...
			if(selectedStudent > 0 && selectedStudent != ''){
				$.get("{{ route('conenStudentSelected') }}",{selectedStudent: selectedStudent},function(objectStudent){
		...
		$('#btnFormSaveConsolidatedEnrollment').on('click',function(e){
			...
			if(idStudent > 0 && idStudent !== ''){
				...
				$.post("{{ route('saveConsolidatedEnrollment') }}", {jsonItems}, function(objectResponse){
