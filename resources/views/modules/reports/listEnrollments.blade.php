...
@section('logisticModules')
	<div class="col-md-12">
		...
		<table id="tableDatatable" class="table table-hover mt-3 tbl-reportStudents" width="100%" style="font-size: 12px; text-align: center;">
			<thead>
				<th>ID</th>
				<th>ALUMNO</th>
				<th>DOCUMENTO</th>
...
@section('scripts')
	<script>
		...
		$('select[name=leCourse]').on('change',function(e){
			...
			if(courseSelected != ''){
				$.get("{{ route('getStudentFromCourseWithEnrollment') }}",{courseSelected: courseSelected},function(objectStudents){
		...
		$('.btn-addStudent').on('click',function(){
			...
			if(course != '' && student != ''){
				$.get("{{ route('getRowStudent') }}",{student: student},function(objectRow){
					if(objectRow != null){
