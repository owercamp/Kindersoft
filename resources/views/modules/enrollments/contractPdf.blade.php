      ...
      if($len < 5 & $len> 0){
        ...
        <div id="content">
          <p style="text-align: justify; font-size: 12px;">
            ...
            <b>CLÁUSULA QUINTA</b>.
            @if(isset($paid))
            COSTOS: El presente contrato por este servicio tendrá un costo anual de <b>${{ number_format($paid->payValueContract,0,',','.') }}</b> que corresponde a <b>${{ number_format($paid->payValuemountContract,0,',','.') }}</b> por concepto de pensión mensual del año lectivo, el cual debe ser cancelado en (<b>{{ $paid->payDuesQuotationContract }}</b>) cuota/s mensuales dentro de los (<b>{{ returnNumberDay($paid->payDatepaidsContract) }}</b>) primeros días de cada mes y <b>${{ number_format($paid->payValueEnrollment,0,',','.') }}</b> por concepto de matrícula. El costo anual se ajustará cada año, según lo autorizado por la Ley.
            @else
            ...
            HORARIOS, EXCURSIONES, SALIDAS Y EVENTOS. HORARIOS: {{ mb_strtoupper($garden->garNamecomercial) }}, está inscrito como calendario B, desde los meses de agosto de @php echo date('Y') @endphp hasta junio de @php echo date('Y') + 1 @endphp. En noviembre tendremos receso previamente informado hasta diciembre de @php echo (date('Y')) @endphp. Parte de noviembre y diciembre @php echo date('Y') @endphp por vacaciones de mitad de año lectivo. El Jardín ofrece vacaciones divertidas por semanas con cobro adicional y de manera voluntaria para el beneficiario que quiera tomarlas; De igual manera también tendremos los recesos de semana Mayor.
            <b>PARÁGRAFO PRIMERO:</b>
