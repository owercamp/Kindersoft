    ...
    <body style="font-size: 12px; text-align: center;">
                ...
                <tbody>
                    ...
                    @for($i = 0; $i < count($consolidated); $i++)
                        @if($consolidated[$i][0] == 'ASISTENCIA')
                            ...
                            @else
                                @for($pp = 0; $pp < count($consolidated[$i][3]); $pp++)
                                    <tr class="{{ $consolidated[$i][3][$pp][0] }}">
        ...
        <div style="width: 100%; border: 1px solid #ccc; padding: 20px; text-align: left;">
            ...
            <ol>
                @php $travel = 0; @endphp
                @for($i = 0; $i < count($consolidated); $i++)
                    @if($consolidated[$i][0] == 'INASISTENCIA')
                        @if($consolidated[$i][2] == 'SIN REGISTROS')
                            <h6 style="font-weight: bold; display: block; width: 100%; height: 30px; background-color: #D4D210;">NO HAY REGISTROS DE INASISTENCIAS</h6>
                        @else
        ...
        <div style="width: 100%; border: 1px solid #ccc; padding: 20px; text-align: center;">
            ...
            @for($i = $travel; $i < count($consolidated); $i++)
                @if($consolidated[$i][0] == 'DETALLES')
                    @if($consolidated[$i][2] != 'SIN REGISTROS')
                        <table width="100%" class="text-center" style="border-collapse: collapse;">
                            ...
                            <tbody>
                                @for($n = 0; $n < count($consolidated[$i][2]); $n++)
                                    <tr>
            ...
            <table width="100%" class="text-center" style="border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    @for($n = 0; $n < count($consolidated); $n++)
                        @if($consolidated[$n][0] == 'EVENTOS')
                            <tr style="padding: 20px; font-size: 10px; text-align: center;">
