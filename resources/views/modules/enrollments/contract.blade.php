...
@section('enrollmentsComercial')
	<div class="col-md-12">
		...
		<table id="tablecontracts" class="table text-center" style="font-size: 13px;">
			<thead>
				<tr>
			...
			<tbody>
				@php $i = 1 @endphp
				@for($row = 0;$row < count($arrayLegalizations);$row++)
					<tr>
...
@section('scripts')
	<script>
		...
		$('.detailsLegalization-link').on('click',function(){
			...
			$.get("{{ route('getPaid') }}",{legId: legId}, function(paidObject){
