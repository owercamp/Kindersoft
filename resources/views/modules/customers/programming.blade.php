							...
							<div class="card-footer">
								<div class="row">
									<div class="col-md-12 text-center">
										<button type="submit" class="bj-btn-table-add form-control-sm">AGENDAR CITA</button>
									</div>
								</div>
							</div>
						</form>
				        	...
				        	<div class="col-md-12">
								<div class="card-body col-md-12">
									...
									<div class="row card-footer text-center bj-active">
										<div class="col-md-6">
											<button type="button" id="confirmScheduling" class="bj-btn-table-edit mt-2 form-control-sm">ASISTIDA</button>
										</div>
										<div class="col-md-6">
											<button type="button" id="cancelScheduling" class="bj-btn-table-delete mt-2 form-control-sm">INASISTIDA</button>
										</div>
									</div>
									<div class="row card-footer text-center bj-inactive">
										<div class="col-md-12">
											<button type="button" class="bj-btn-table-delete mt-2 form-control-sm" data-dismiss="modal">CERRAR</button>
										</div>
									</div>
								</div>
				        	</div>
	...
	<script>
		$(function(){
			...
			$("#schCustomer_id").on("change", function(e){
				...
				$.get("{{ route('subcustomer') }}", {customer_id: customer_id}, function(customerObject){
		...
		document.addEventListener('DOMContentLoaded', function() {
	        ...
	        var calendar = new FullCalendar.Calendar(calendarEl, {
				...
				eventClick: function(info){
					...
					$.get("{{ route('allscheduling') }}", {scheduling: info.event.id}, function(schedulingObject){
					...
					$('#confirmScheduling').on('click',function(e){
						e.preventDefault();
						$.get("{{ route('confirmScheduling') }}", {schId: $('.see-schId').val() },function(response){
							$("#seeEvent").modal('hide');
							//info.refetchEvents();
						});
					});
					...
					$('#cancelScheduling').on('click',function(e){
						e.preventDefault();
						$.get("{{ route('cancelScheduling') }}", {schId: $('.see-schId').val() },function(response){
							$("#seeEvent").modal('hide');
							//info.refetchEvents();
						});
					});
					...
					if(dateComplete <= dateDroped){
						...
						$('#confirmNewHour').on('click',function(){
							if($('input[name=new-schHourVisit]').val() !== ''){
								$.get("{{ route('reschedule') }}", {schId: info.event.id, newDate: dateDroped, newHour: $('input[name=new-schHourVisit]').val()},function(response){
