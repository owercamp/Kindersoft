			...
			<div class="col-md-6">
				<form action="{{ route('health.new') }}" method="PUT">
					...
					<div class="row justify-content-center">
						<button type="submit" class="bj-btn-table-add my-2 form-control-sm">CREAR</button>
					</div>
				</form>
			</div>
			<div class="col-md-6">
			...
			<tbody>
				...
				@foreach($healths as $health)
					<tr>
						...
						<td>{{ $health->entity }}</td>
						<td>{{ $health->type }}</td>
						<td><a href="{{ route('health.edit', $health->id) }}" title="EDITAR" class="bj-btn-table-edit"><i class="fas fa-edit"></i></a></td>
						<td><a href="{{ route('health.delete', $health->id) }}" title="ELIMINAR" class="bj-btn-table-delete" onclick="return confirm('- Se eliminará el centro de salud')"><i class="fas fa-trash-alt"></i></a></td>
					</tr>
				@endforeach
			</tbody>
			<tfoot>
