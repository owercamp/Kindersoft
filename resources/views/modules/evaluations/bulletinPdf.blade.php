  ...
  if ( $mountNow < $mount ){ $old--; } if ($mount==$mountNow && $dayNow <$day){ $old--; } if ($old> 1900){ $old -= 1900; }
    ...
    if($mountNow>$mount && $day > $dayNow){ $mounts=($mountNow-$mount)-1; }
      ...
      if($len < 5 & $len> 0){
        ...
        @for($i = 0; $i < count($infoIntelligence); $i++) @php $findIntelligence=strpos($validateRepeat,(string)$infoIntelligence[$i][0]); @endphp @if($findIntelligence===false) <section>
          <table width="100%" border="1" style="font-size: 11px; margin-top: 10px; max-width: 100%;">
            ...
            <tbody>
              ...
              @for($m = 0; $m < count($infoAchievement); $m++) @if($infoAchievement[$m][0]==$infoIntelligence[$i][0]) @php $countAchievementFromThisIntelligence++; @endphp @endif @endfor @for($a=0; $a < count($infoAchievement); $a++) @if($infoAchievement[$a][0]==$infoIntelligence[$i][0]) @php $validate++; @endphp @php $itemsPercentaje++; @endphp <tr>
                @if(!$validateOther)
          ...
          <section>
            <table width="100%" border="1" style="font-size: 12px; max-width: 100%;">
              <thead>
                ...
                @for($in = 0; $in < count($resultIntelligences); $in++) <tr>
                  @if($in == 0)
