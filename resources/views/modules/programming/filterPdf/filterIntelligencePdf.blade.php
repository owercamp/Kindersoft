...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <header>
            <table width="100%">
                <tr>
                    <td class="text-right"><small class="text-muted text-right"><b>Inteligencia:</b></small></td>
                    <td class="text-left"><small class="text-muted text-left">{{ $intelligence->type }}</small></td><!-- Nombre de inteligencia -->
                    <td colspan="2" class="text-right">
                        <small class="text-muted text-right">CRONOGRAMA POR ASIGNATURA/INTELIGENCIA</small>
                    </td>
                </tr>
                <tr>
                    <!-- <td class="text-right"><small class="text-muted text-right"><b>Rango de fechas:</b></small></td>
                    <td class="text-left"><small class="text-muted text-left"> {{ $dateInitial }} A {{ $dateFinal }} </small></td> --><!-- Rango de fechas -->
        ...
        <table width="100%" class="text-center">
            <tr>
                <td><small class="text-muted"><b>CURSO</b></small></td>
            ...
            @foreach($resultFilter as $result)
                <tr>
                    <td><small class="text-muted">{{ $result->nameCourse }}</small></td>
                    <td><small class="text-muted">{{ $result->apNameperiod }}</small></td>
                    @php
                        $separatedRange = explode('/',$result->chRangeWeek);
                        $getDateInitialString = getDateString($separatedRange[0]);
                        $getDateFinalString = getDateString($separatedRange[1]);
                    @endphp
