				...
				<form action="{{ route('district.new') }}" method="PUT">
					<div class="row">
						...
						<div class="col-md-6">
							...
							<div class="form-group">
							    <button type="submit" class="bj-btn-table-add mt-2 form-control-sm">CREAR</button>
							</div>
						</div>
					</div>
				</form>
			...
			<tbody>
				...
				@foreach($districts as $district)
					<tr>
						<td>{{ $i++ }}</td>
						<td>{{ $district->name }}</td>
						<td><a href="{{ route('district.edit', $district->id) }}" title="EDITAR" class="bj-btn-table-edit"><i class="fas fa-edit"></i></a></td>
						<td><a href="{{ route('district.delete', $district->id) }}" title="ELIMINAR" class="bj-btn-table-delete" onclick="return confirm('¿Desea borrar el barrio?')"><i class="fas fa-trash-alt"></i></a></td>
					</tr>
				@endforeach
			</tbody>
			<tfoot>
	...
	<script>
		$(document).ready(function(){
			$("#city").on("change", function(e){
				...
				$.get("{{ route('edit.sublocation') }}", {cityhome_id: city_id}, function(locationObject){
