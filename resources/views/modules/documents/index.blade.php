  ...
  <table id="tabledocuments" class="table table-hover text-center" width="100%">
    ...
    <tbody>
      ...
      <tr>
        <td>{{ $i++ }}</td>
        <td>{{ $document->type }}</td>
        <td><a href="{{ route('document.edit', $document->id) }}" title="EDITAR" class="btn btn-outline-primary rounded-circle"><i class="fas fa-edit"></i></a></td>
        <td><a href="{{ route('document.delete', $document->id) }}" title="ELIMINAR" class="btn btn-outline-tertiary rounded-circle " onclick="return confirm('- Se borrará el tipo de documento')"><i class="fas fa-trash-alt"></i></a></td>
      </tr>
      @endforeach
    </tbody>
    <tfoot>
