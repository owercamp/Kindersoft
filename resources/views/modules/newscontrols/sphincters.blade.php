...
@section('logisticModules')
	<div class="col-md-12">
		<div class="row text-center my-2">
			<div class="col-md-6">
				<h3>MODULO DE CONTROL DE ESFINTERES</h3>
			</div>
			<div class="col-md-6">
				<!-- Mensajes de creación de esfinteres -->
				@if(session('PrimarySuccessSphincters'))
				    <div class="alert alert-success">
				        {{ session('PrimarySuccessSphincters') }}
				    </div>
				@endif
				@if(session('SecondarySuccessSphincters'))
				    <div class="alert alert-success">
				        {{ session('SecondarySuccessSphincters') }}
				    </div>
				@endif
				<!-- Mensajes de actualizacion de esfinteres -->
				@if(session('PrimaryUpdateSphincters'))
				    <div class="alert alert-primary">
				        {{ session('PrimaryUpdateSphincters') }}
				    </div>
				@endif
				@if(session('SecondaryUpdateSphincters'))
				    <div class="alert alert-secondary">
				        {{ session('SecondaryUpdateSphincters') }}
				    </div>
				@endif
				<!-- Mensajes de eliminación de esfinteres -->
				@if(session('WarningDeleteSphincters'))
				    <div class="alert alert-warning">
				        {{ session('WarningDeleteSphincters') }}
				    </div>
				@endif
				@if(session('SecondaryDeleteSphincters'))
				    <div class="alert alert-secondary">
				        {{ session('SecondaryDeleteSphincters') }}
				    </div>
				@endif
				<a href="#" class="bj-btn-table-edit newSphincters-link">NUEVO CONTROL</a>
			</div>
		</div>
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-header">
	        <div class="row p-3 m-3">
	        	<div class="col-md-12">
	        		<h5 class="text-muted">NOVEDADES DE ESFINTERES:</h5>
	        	</div>
	        </div>
	  ...
	  <div class="modal-dialog modal-lg">
	    <div class="modal-content">
	      ...
	      <div class="modal-header">
	        <div class="row p-3 m-3">
	        	<div class="col-md-12">
	        		<h5 class="text-muted">NUEVA NOVEDADES DE CONTROL DE ESFINTERES:</h5>
	        	</div>
	        </div>
	        <button type="button" class="close" data-dismiss="modal">&times;</button>
	      </div>
	      	...
	      	<div class="row modal-body">
	      		<div class="col-md-12">
					<form action="{{ route('sphincters.new') }}" method="POST" autocomplete="off">
						@csrf
						<div class="row p-3">
		      				<div class="col-md-3">
		      			...
		      			<div class="row p-3">
		      				<div class="col-md-12">
		      					...
		      					<textarea name="newControl_NewSphinters" placeholder="ESCRIBA LA NOVEDAD DE ESFINTER" class="form-control form-control-sm" required></textarea>
		      				</div>
		      			</div>
...
@section('scripts')
	<script>
		...
		$('.newSphincters-link').on('click',function(e){
			e.preventDefault();
