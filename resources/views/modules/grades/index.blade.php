			...
			<tbody>
				...
				@foreach($grades as $grade)
					<tr>
						<td>{{ $i++ }}</td>
						<td>{{ $grade->name }}</td>
						<td><a href="{{ route('grade.edit', $grade->id) }}" title="EDITAR" class="bj-btn-table-edit"><i class="fas fa-edit"></i></a></td>
						<td><a href="{{ route('grade.delete', $grade->id) }}" title="ELIMINAR" class="bj-btn-table-delete" onclick="return confirm('Si borra este grado, se eliminarán los cursos relacionados. ¿Desea borrar el grado?')"><i class="fas fa-trash-alt"></i></a></td>
					</tr>
				@endforeach
			</tbody>
			<tfoot>
