...
@section('logisticModules')
	<div class="col-md-12">
		<div class="row text-center my-2">
			<div class="col-md-6">
				<h3>MODULO DE CONTROL DE ALIMENTACION</h3>
			</div>
			<div class="col-md-6">
				<!-- Mensajes de creación de alimentación -->
				@if(session('SuccessCreateFeedings'))
				    <div class="alert alert-success">
				        {{ session('SuccessCreateFeedings') }}
				    </div>
				@endif
				@if(session('SecondaryCreateFeedings'))
				    <div class="alert alert-secondary">
				        {{ session('SecondaryCreateFeedings') }}
				    </div>
				@endif
				<!-- Mensajes de actualizacion de alimentación -->
				@if(session('PrimaryUpdateFeedings'))
				    <div class="alert alert-primary">
				        {{ session('PrimaryUpdateFeedings') }}
				    </div>
				@endif
				@if(session('SecondaryUpdateFeedings'))
				    <div class="alert alert-secondary">
				        {{ session('SecondaryUpdateFeedings') }}
				    </div>
				@endif
				<!-- Mensajes de eliminación de alimentación -->
				@if(session('WarningDeleteFeedings'))
				    <div class="alert alert-warning">
				        {{ session('WarningDeleteFeedings') }}
				    </div>
				@endif
				@if(session('SecondaryDeleteFeedings'))
				    <div class="alert alert-secondary">
				        {{ session('SecondaryDeleteFeedings') }}
				    </div>
				@endif
				<a href="#" class="bj-btn-table-edit newFeeding-link">NUEVO CONTROL</a>
			</div>
		</div>
			...
			<tbody>
				@for($i = 0; $i < count($result); $i++)
					<tr>
						...
						<td>
							<a href="#" title="VER NOVEDADES" class="bj-btn-table-add seeNews-link">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-header">
	        <div class="row p-3 m-3">
	        	<div class="col-md-12">
	        		<h5 class="text-muted">NOVEDADES DE ALIMENTACION:</h5>
	        	</div>
	        </div>
				        	...
				        	<table class="table table-hover table-news" width="100%">
				        		<thead>
									<tr>
										...
										<th style="max-width: 50px;">TIPO DE NOVEDAD</th>
										<th>NOVEDAD</th>
									</tr>
								</thead>
								<tbody>
	  ...
	  <div class="modal-dialog modal-lg">
	    <div class="modal-content">
	      ...
	      <div class="modal-header">
	        <div class="row p-3 m-3">
	        	<div class="col-md-12">
	        		<h5 class="text-muted">NUEVA NOVEDADES DE CONTROL DE ALIMENTACION:</h5>
	        	</div>
	        </div>
	        <button type="button" class="close" data-dismiss="modal">&times;</button>
	      </div>
	      	...
	      	<div class="row modal-body">
	      		<div class="col-md-12">
					<form action="{{ route('feedings.new') }}" method="POST" autocomplete="off">
						@csrf
						<div class="row p-3">
		      				<div class="col-md-4">
		      			...
		      			<div class="row p-3">
		      				...
		      				<div class="col-md-6">
		      					...
		      					<select name="newControl_type" class="form-control form-control-sm" required>
		      						...
		      						<option value="Refrigerio PM">Refrigerio PM</option>
		      					</select>
		      				</div>
		      			</div>
		      			<div class="row p-3">
		      				<div class="col-md-12">
		      					...
		      					<textarea name="newControl_NewFeedings" placeholder="ESCRIBA LA NOVEDAD DE ALIMENTACION" class="form-control form-control-sm" required></textarea>
		      				</div>
		      			</div>
		      			<div class="row text-center">
		      				<div class="col-md-12">
		      					<button type="submit" class="bj-btn-table-add">GUARDAR NOVEDAD</button>
		      				</div>
		      			</div>
					</form>	
	      		</div>
	...
	<script>
		...
		$('.seeNews-link').on('click',function(e){
			...
			for (var i = 0; i < separatedNews.length; i++) {
				var separatedType = separatedNews[i].split('=|=');
				$('.table-news tbody').append("<tr>" +
							"<td>" + rows + "</td>" +
							"<td>" + separatedType[0] + "</td>" +
							"<td>" + separatedType[1] + "</td>" +
						"</tr>");
				rows++;
			}
			$('#news-modal').modal();
		...
		$('select[name=newControl_Grade]').on('change',function(e){
			...
			if(grade !== null && grade !== ''){
				...
				$.get("{{ route('legGradeSelected') }}",{selectedGrade: grade},function(objectCourses){
		...
		$('select[name=newControl_Course]').on('change',function(e){
			...
			if(grade !== null && grade !== '' && course !== null && course !== ''){
				...
				$.get(
					...
					{selectedCourse: course, selectedGrade: grade},
		...
		function getFormatDate(date){
			...
			switch(mount){
				case '01': return 'Enero ' + day + ' del ' + year; break;
				case '02': return 'Febrero ' + day + ' del ' + year; break;
				case '03': return 'Marzo ' + day + ' del ' + year; break;
				case '04': return 'Abril ' + day + ' del ' + year; break;
				case '05': return 'Mayo ' + day + ' del ' + year; break;
				case '06': return 'Junio ' + day + ' del ' + year; break;
				case '07': return 'Julio ' + day + ' del ' + year; break;
				case '08': return 'Agosto ' + day + ' del ' + year; break;
				case '09': return 'Septiembre ' + day + ' del ' + year; break;
				case '10': return 'Octubre' + day + ' del ' + year; break;
				case '11': return 'Noviembre ' + day + ' del ' + year; break;
				case '12': return 'Diciembre ' + day + ' del ' + year; break;
			}
		}
	</script>
@endsection